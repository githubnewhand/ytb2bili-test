...
func (h *AuthHandler) getQRCode(c *gin.Context) {
	...
	// Prefer the exact URL returned by the upstream API. Reconstructing it from
	// auth_code is brittle and can produce an unusable QR code when the upstream
	// format changes.
	qrCodeContent := qrResp.Data.URL
	if qrCodeContent == "" {
		qrCodeContent = fmt.Sprintf("https://passport.bilibili.com/x/passport-tv-login/h5/qrcode/auth?auth_code=%s", qrResp.Data.AuthCode)
	}

	qrCodePath := fmt.Sprintf(
		"/api/v1/auth/qrcode/image/%s?url=%s",
		qrResp.Data.AuthCode,
		url.QueryEscape(qrCodeContent),
	)
